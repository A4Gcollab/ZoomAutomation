...
YOUTUBE_TOKEN_PATH = SECRETS_DIR / "token_youtube.json"
...
def check_config():
    ...
    if not YOUTUBE_CLIENT_SECRET_PATH.exists():
        raise ValueError(f"YouTube client secret not found: {YOUTUBE_CLIENT_SECRET_PATH}")
    
    if DRIVE_AUTH_MODE == "service_account" and not DRIVE_SERVICE_ACCOUNT_FILE.exists():
        raise ValueError(f"Drive service account file not found: {DRIVE_SERVICE_ACCOUNT_FILE}")
